...
let rec adjacent_product symbol pparts acc =
  ...
  | [] -> acc
  | head :: tail -> 
      if check_poximity (fst (fst head)) symbol = true || check_poximity (snd (fst head)) symbol = true then
        adjacent_product symbol tail (fst acc + 1, snd acc * (snd head))
      else
        adjacent_product  symbol tail acc
...
let rec count pparts symbols acc =
  ...
  | head :: tail ->
      ...
      Printf.printf "Found %d occurrence, giving %d for s(%d,%d)\n" occurrence result (fst head) (snd head);
      if occurrence = 2 then
        count pparts tail (acc + result)
      else
        count pparts tail acc
